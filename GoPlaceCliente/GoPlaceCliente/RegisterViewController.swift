...
class RegisterViewController:UIViewController{
    ...
    @IBAction func clickBtnRegister(_ sender: Any) {
        ...
        if let email = txtFieldEmail.text, let password = txtFieldPassword.text, let name = txtFieldName.text, let lastName = txtFieldLastName.text, let phone = txtFieldPhone.text{
            Auth.auth().createUser(withEmail: email, password: password){
                (result, error) in
                
                if let result = result, error == nil{
                    
                    self.saveDataUser(name: name, lastName: lastName, email: email, phone: phone)
    ...
    private func saveDataUser(name:String,lastName:String, email:String,phone:String){
